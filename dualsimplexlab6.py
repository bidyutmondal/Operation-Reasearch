# -*- coding: utf-8 -*-
"""dualSimplexLab6.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1EboMqseHtG5xAxmdG1T2oyU8XT8ecncj
"""
...
def primalToDual():
    t_var = [0.0]*m
    for i in range(0,m): t_var[i] = float(arr[i][n])
    n_arr = np.empty((n, m+1))
    for i in range(0,n):
        for j in range(0,m): n_arr[i][j] = arr[j][i]
    for i in range(0,n): n_arr[i][m] = obj_arr[i]
    return (n_arr, t_var)
...
def readInput():
	...
	obj = input('\n\nEnter the coefficient of variables in objective function(separated by space):')
	obj_arr = obj.split()
	...
	for i in range (m):
		...
		sgn = raw_arr.pop(n)
		if (sgn == ">=" and mi==1):
		    for j in range(0,n+1): raw_arr[j] = float(raw_arr[j])*-1
